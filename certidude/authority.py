...
from certidude import push, mailer
...
class Authority:
    def __init__(self, config):
        self.config = config
        with open(self.config.AUTHORITY_CERTIFICATE_PATH, "rb") as fh:
            certificate_buf = fh.read()
            header, _, certificate_der_bytes = pem.unarmor(certificate_buf)
            self.certificate = x509.Certificate.load(certificate_der_bytes)
            self.public_key = asymmetric.load_public_key(self.certificate["tbs_certificate"]["subject_public_key_info"])
        with open(self.config.AUTHORITY_PRIVATE_KEY_PATH, "rb") as fh:
            key_buf = fh.read()
            header, _, key_der_bytes = pem.unarmor(key_buf)
            self.private_key = asymmetric.load_private_key(key_der_bytes)
    ...
    def self_enroll(self):
        ...
        try:
            path, buf, cert, signed, expires = self.get_signed(common_name)
        ...
        if not pid:
            ...
            drop_privileges()
            self.sign(common_name, skip_push=True, overwrite=True)
            sys.exit(0)
        else:
            os.waitpid(pid, 0)
    ...
    def get_signed(self, common_name):
        if not re.match(RE_HOSTNAME, common_name):
            raise ValueError("Invalid common name %s" % repr(common_name))
        path = os.path.join(self.config.SIGNED_DIR, common_name + ".pem")
        with open(path, "rb") as fh:
            buf = fh.read()
            header, _, der_bytes = pem.unarmor(buf)
    ...
    def get_revoked(self, serial):
        ...
        path = os.path.join(self.config.REVOKED_DIR, "%x.pem" % serial)
        with open(path, "rb") as fh:
            buf = fh.read()
            header, _, der_bytes = pem.unarmor(buf)
            cert = x509.Certificate.load(der_bytes)
    ...
    def get_attributes(self, cn, namespace=None):
        path, buf, cert, signed, expires = self.get_signed(cn)
        attribs = dict()
    ...
    def store_request(self, buf, overwrite=False, address="", user=""):
        """
        ...
        request_path = os.path.join(self.config.REQUESTS_DIR, common_name + ".pem")
    ...
    def revoke(self, common_name):
        """
        Revoke valid certificate
        """
        signed_path, buf, cert, signed, expires = self.get_signed(common_name)
        revoked_path = os.path.join(self.config.REVOKED_DIR, "%x.pem" % cert.serial_number)

        os.unlink(os.path.join(self.config.SIGNED_BY_SERIAL_DIR, "%x.pem" % cert.serial_number))
        ...
        url = self.config.LONG_POLL_PUBLISH % "crl"
        click.echo("Publishing CRL at %s ..." % url)
        requests.post(url, data=self.export_crl(),
    ...
    def server_flags(self, cn):
        if self.config.USER_ENROLLMENT_ALLOWED and not self.config.USER_MULTIPLE_CERTIFICATES:
    ...
    def list_requests(self, directory=None):
        d = directory or self.config.REQUESTS_DIR
        for filename in os.listdir(d):
            if filename.endswith(".pem"):
                common_name = filename[:-4]
                path, buf, req, submitted = self.get_request(common_name)
                yield common_name, path, buf, req, submitted, "." in common_name

    def _list_certificates(self, directory):
    ...
    def list_signed(self, directory=None):
        d = directory or self.config.SIGNED_DIR
        for filename in os.listdir(d):
            if filename.endswith(".pem"):
                common_name = filename[:-4]
                path, buf, cert, signed, expires = self.get_signed(common_name)
    ...
    def list_revoked(self, directory=None):
        d = directory or self.config.REVOKED_DIR
        for filename in os.listdir(d):
            if filename.endswith(".pem"):
                common_name = filename[:-4]
                path, buf, cert, signed, expired, revoked = self.get_revoked(common_name)
                yield cert.subject.native["common_name"], path, buf, cert, signed, expired, revoked

    def list_server_names(self):
        return [cn for cn, path, buf, cert, server in self.list_signed() if server]

    def export_crl(self, pem=True):
        builder = CertificateListBuilder(
            self.config.AUTHORITY_CRL_URL,
            self.certificate,
            1 # TODO: monotonically increasing
        )

        for filename in os.listdir(self.config.REVOKED_DIR):
            if not filename.endswith(".pem"):
                continue
            serial_number = filename[:-4]
            # TODO: Assert serial against regex
            revoked_path = os.path.join(self.config.REVOKED_DIR, filename)
        ...
        certificate_list = builder.build(self.private_key)
        if pem:
            return pem_armor_crl(certificate_list)
        return certificate_list.dump()


    def delete_request(self, common_name):
        # Validate CN
        if not re.match(RE_HOSTNAME, common_name):
            raise ValueError("Invalid common name")

        path, buf, csr, submitted = self.get_request(common_name)
        os.unlink(path)
        ...
        requests.delete(
            self.config.LONG_POLL_PUBLISH % hashlib.sha256(buf).hexdigest(),
            headers={"User-Agent": "Certidude API"})

    def sign(self, common_name, skip_notify=False, skip_push=False, overwrite=False, profile="default", signer=None):
        """
        Sign certificate signing request by it's common name
        """

        req_path = os.path.join(self.config.REQUESTS_DIR, common_name + ".pem")
        ...
        cert, buf = self._sign(csr, csr_buf, skip_notify, skip_push, overwrite, profile, signer)

        os.unlink(req_path)
        return cert, buf

    def _sign(self, csr, buf, skip_notify=False, skip_push=False, overwrite=False, profile="default", signer=None):
        ...
        common_name = csr["certification_request_info"]["subject"].native["common_name"]
        cert_path = os.path.join(self.config.SIGNED_DIR, "%s.pem" % common_name)
        renew = False
        ...
        if os.path.exists(cert_path):
            ...
            if overwrite:
                ...
                revoked_path = os.path.join(self.config.REVOKED_DIR, "%s.pem" % prev_serial_hex)
        ...
        profile_server_flags, lifetime, dn["organizational_unit_name"], _ = self.config.PROFILES[profile]
        ...
        end_entity_cert = builder.build(self.private_key)
        ...
        # Create symlink
        link_name = os.path.join(self.config.SIGNED_BY_SERIAL_DIR, "%x.pem" % end_entity_cert.serial_number)
        ...
        if not skip_push:
            url = self.config.LONG_POLL_PUBLISH % hashlib.sha256(buf).hexdigest()
